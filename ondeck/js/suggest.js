    ...
    if ((query == null) | (query.trim() == "")) {
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          var clearR = "<span style='float:right;font-size:10px;padding:5px 0px 0 0;text-transform: uppercase;font-weight: normal;'><a href=\"javascript:void('')\" onClick=\"$('#response_complete').html('');\">Clear Results</a></span>";
          ...
          if (data["status"] == 200) {
            ...
            if (data["response"].includes("[NO SUPPORT FOUND]")) {
              $('#response_complete').html(clearR+"<h2>No Support Found</h2><p>We couldn't find enough supporting material in your library to suggest any text. Consider writing more and trying again.</p>");
            } else {
              $('#seed').val($('#seed').val().trim()+" "+data["response"].trim());
